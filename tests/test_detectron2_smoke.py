...
from engine.catalog import detectron2_installed, ensure_pretrained
...
@pytest.mark.skipif(not detectron2_installed(), reason="Detectron2 is not installed")
@pytest.mark.skipif(not _cuda(), reason="CUDA is not visible in this environment")
def test_short_detectron2_train_and_infer(tmp_path: Path) -> None:
    images_dir, image_name, width, height = _prepare_images(tmp_path)
    annotations = tmp_path / "annotations.json"
    _write_coco(annotations, image_name, width, height)
    pretrained = ensure_pretrained(Path(os.environ.get("AITEM_WEIGHTS_DIR", "/data/weights")))
